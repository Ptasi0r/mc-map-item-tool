    ...
    try {
      ...
      zlib.gzip(nbtData, function (err, data) {
        ...
        let writeStream = new lazystream.Writable(function () {
          return fs.createWriteStream(`${TMP_DIR}${filename}.dat`).on('close', function () {
  ...
  createZipFile(req, res, body) {
    try {
      ...
      for (let [index, element] of mapfiles.entries()) {
        ...
        archive.file(`${TMP_DIR}${element}.dat`, {
          name: `map_${filenumber}.dat`
        });
      }
...
function startup() {
  ...
  logger.log('info', `Started at: ${process.env.HOST}:${process.env.PORT || 8080}`);
}
